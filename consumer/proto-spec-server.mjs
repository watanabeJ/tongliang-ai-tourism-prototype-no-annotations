...
async function handleSpecRead(res, pageKey) {
  const resolved = await resolveSpec(pageKey);
  const markdown = resolved.markdown;
  if (!markdown) {
    res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8", "Access-Control-Allow-Origin": "*" });
    res.end(`Spec not found: ${safePageKey(pageKey)}`);
    return;
  }
...
async function writeManualSpec(pageKey, markdown) {
  const resolved = await resolveSpec(pageKey);
  const safeKey = resolved.canonicalKey || safePageKey(pageKey);
  const currentDir = path.join(root, "prototype-annotator", "specs", "current");
  const historyDir = path.join(root, "prototype-annotator", "specs", "history", safeKey);
  const currentFile = resolved.file || path.join(currentDir, safeKey + ".md");
...
async function resolveSpec(pageKey) {
  const requestedKey = safePageKey(pageKey);
  const currentDir = path.join(root, "prototype-annotator", "specs", "current");
  const registryPath = path.join(root, "prototype-annotator", "specs", "registry.json");
  const registry = await fs.readFile(registryPath, "utf-8").then((raw) => JSON.parse(raw)).catch(() => null);
  const pages = Array.isArray(registry?.pages) ? registry.pages : [];
  const entry = pages.find((item) => {
    const pathKey = String(item.path || "").replace(/\.html?$/i, "");
    const routeKey = String(item.route || "").replace(/^\//, "").replace(/\.html?$/i, "");
    return String(item.pageKey || "") === requestedKey || pathKey === requestedKey || routeKey === requestedKey;
  });
  const canonicalKey = safePageKey(entry?.pageKey || requestedKey);
  const file = path.join(currentDir, canonicalKey + ".md");
  const markdown = await fs.readFile(file, "utf-8").catch(() => "");
  return { canonicalKey, file, markdown };
}
